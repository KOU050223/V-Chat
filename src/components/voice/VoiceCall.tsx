...
export default function VoiceCall({
    ...
    const [isAudioMonitoringActive, setIsAudioMonitoringActive] =
        useState(false);
    ...
    const startContinuousAudioMonitoring = async () => {
        try {
            ...
            console.error(
                'Failed to start continuous audio monitoring:',
                error
            );
    ...
    const connectToRoom = async () => {
        ...
        try {
            ...
            try {
                ...
                console.error(
                    'Failed to get access token from Cloud Functions:',
                    tokenError
                );

                // エラーメッセージを詳細に表示
                let errorMessage = 'トークンの取得に失敗しました';
                if (tokenError.code === 'unauthenticated') {
                    errorMessage = '認証が必要です。ログインしてください。';
                } else if (tokenError.code === 'not-found') {
                    errorMessage = 'ルームが見つかりません。';
                } else if (tokenError.code === 'permission-denied') {
                    errorMessage = 'このルームへの参加権限がありません。';
                } else if (tokenError.message) {
                    errorMessage = `エラー: ${tokenError.message}`;
                }
            ...
            try {
                ...
                try {
                    ...
                    const stream =
                        await navigator.mediaDevices.getUserMedia(constraints);
                    console.log(
                        'Microphone permission granted with selected device'
                    );
                ...
                const existingParticipants = allRemoteParticipants.filter(
                    (p) => {
                        const participantBaseName = p.identity
                            ? p.identity.split('-')[0]
                            : '';
                        ...
                        console.log(
                            `  - SID: ${p.sid} vs ${newRoom.localParticipant?.sid}`
                        );
                        console.log(
                            `  - Identity: ${p.identity} vs ${newRoom.localParticipant?.identity}`
                        );
                        console.log(
                            `  - Base name: ${participantBaseName} vs ${myBaseName}`
                        );
                        return !isMyself;
                    }
                );
            ...
            if (err instanceof Error) {
                if (err.message.includes('could not establish pc connection')) {
                    ...
                    err.message.includes('LiveKit URL is not configured')
                ) {
                    errorMessage =
            ...
            if (process.env.NODE_ENV === 'development') {
                console.warn(
                    '開発環境: LiveKit接続失敗ですが、画面表示を続行します'
                );
    ...
    const handleParticipantConnected = (participant: RemoteParticipant) => {
        // 自分の名前（タイムスタンプ部分を除く）を取得
        const myBaseName = participantName.split('-')[0];
        const participantBaseName = participant.identity
                                ...
                                <div className="mt-3 flex justify-end space-x-2">
                                    ...
                                    <Button
                                        onClick={async () => {
                                            if (isConnected && room) {
                                                console.log(
                                                    '🔄 Manual audio monitoring restart...'
                                                );
                                                await restartAudioMonitoring();
                                            } else {
                                                console.log(
