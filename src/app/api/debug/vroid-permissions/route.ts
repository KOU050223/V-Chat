  ...
  try {
    ...
    const apiEndpoints = [
      ...
      },
      {
        ...
        description:
          "ダウンロードライセンス取得テスト（存在しないモデルIDでの権限確認）",
        required: false,
    ...
    if (!results.apiTests["Account Info"]?.result?.success) {
      recommendations.push(
        "基本的なアカウント情報の取得に失敗しています。VRoid Hub Developer Consoleでアプリケーション設定を確認してください。"
      );
    }

    if (!results.apiTests["My Character Models"]?.result?.success) {
      recommendations.push(
        "マイモデル一覧の取得権限がありません。VRoid Hub Developer Consoleでアプリケーションの審査を申請するか、権限タイプを変更してください。"
      );
    }

    if (results.summary.successful === 0) {
      recommendations.push(
        "すべてのAPIテストが失敗しました。VRoid Hub Developer Consoleでアプリケーション設定（Client ID、Client Secret、リダイレクトURI）を確認してください。"
      );
    }
