...
export function useVRMDownload(options: UseVRMDownloadOptions = {}) {
  ...
  // キャッシュからVRMを取得
  const getCachedVRM = useCallback(async (modelId: string): Promise<string | null> => {
  ...
  const downloadVRM = useCallback(async (model: VRoidCharacterModel): Promise<string> => {
  ...
  const downloadVRMFile = useCallback(async (model: VRoidCharacterModel): Promise<void> => {
    const modelId = model.id;
  ...
  // ダウンロード状況を取得
  const getDownloadStatus = useCallback((modelId: string): DownloadProgress | null => {
  ...
  const getCacheInfo = useCallback(async () => {
    if (!enableCache || !cacheInitialized) {
      return { count: 0, totalSize: 0, maxSize: maxCacheSize * 1024 * 1024, entries: [] };
    }
