  ...
  try {
    ...
    if (!session?.accessToken) {
      const result = {
        ...
        suggestion: 'VRoid認証が完了していないか、セッションが期限切れです。再ログインが必要です。',
      };
    ...
    if (vroidResponse.ok) {
      ...
      console.log('VRoid API Success - User data received:', JSON.stringify(userData, null, 2));
      result.success = true;
      ...
      result.sessionData = {
        ...
        extractionDetails: {
          ...
          dataStructureAnalysis: (session.vroidProfile as any)?.extractedInfo?.dataStructure,
        }
      };
    } else {
    ...
    return NextResponse.json(
      { 
        ...
        suggestion: 'サーバーエラーが発生しました。コンソールログを確認してください。',
      },
      { status: 500 }
    );
  }
