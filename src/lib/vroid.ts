...
export class VRoidAPI {
  ...
  private async authenticatedRequest<T>(
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        if (
          errorText.includes("OAUTH_FORBIDDEN") ||
          errorText.includes("OAuth")
        ) {
          throw new Error(
            `VRoid API OAuth認証エラー (403): VRoid Hub Developer Consoleでアプリケーション設定を確認してください。特に、リダイレクトURIとスコープ設定をチェックしてください。`
          );
        } else {
          throw new Error(
            `VRoid API アクセス権限がありません (403): このAPIエンドポイント(${endpoint})へのアクセス権限がありません。VRoid Hub Developer Consoleでアプリケーションの権限設定を確認してください。`
          );
        }
      }
      ...
      if (response.status === 401) {
        throw new Error(
          `VRoid API 認証エラー (401): アクセストークンが無効または期限切れです。再ログインが必要です。`
        );
      }

      throw new Error(
        errorData.error ||
          `VRoid API request failed: ${response.status} ${response.statusText}`
      );
    }
    ...
    if (result.error && result.error.message && result.error.code) {
      ...
      switch (errorCode) {
        case "OAUTH_FORBIDDEN":
          throw new Error(
            `OAuth認証エラー: ${errorMessage}. VRoid Hubのアプリケーション設定（リダイレクトURI、スコープ）を確認してください。`
          );
        case "INVALID_TOKEN":
          throw new Error(
            `無効なアクセストークン: ${errorMessage}. 再認証が必要です。`
          );
        case "INSUFFICIENT_SCOPE":
          throw new Error(
            `権限不足: ${errorMessage}. 必要なスコープが不足しています。`
          );
        default:
  ...
  async getMyCharacterModels(
    ...
    return this.authenticatedRequest(`/character_models?${params.toString()}`);
  }
  ...
  async getLikedCharacterModels(options: {
    ...
    if (options.characterization_allowed_user) {
      params.append(
        ...
        options.characterization_allowed_user
      );
    }
    ...
    if (options.corporate_commercial_use) {
      params.append(
        ...
        options.corporate_commercial_use
      );
    }
   ...
   */
  async getCharacterModelDownloadLicense(
    modelId: string
  ): Promise<VRoidAPIResponse<VRoidDownloadLicense>> {
    ...
    try {
      ...
      const result = await this.authenticatedRequest<
        VRoidAPIResponse<VRoidDownloadLicense>
      >(`/download_licenses`, {
        method: "POST",
        body: JSON.stringify({
      ...
      console.log(
        "POST /api/download_licenses エラー、GET方式にフォールバック:",
        postError.message
      );
      ...
      try {
        ...
        if (
          ...
          postError.message.includes("403")
        ) {
          throw new Error(
            `モデル ${modelId} のダウンロード権限がありません。このモデルはダウンロード不可能に設定されているか、あなたがいいねしていない可能性があります。現在のアプリ設定では、いいねしたモデルのみダウンロード可能です。`
          );
        } else if (error.message.includes("404")) {
          throw new Error(
            `モデル ${modelId} が見つかりません。モデルIDを確認してください。`
          );
        } else {
          throw new Error(
            `ダウンロードライセンス取得に失敗しました: ${error.message}`
          );
        }
      }
    }
  ...
  async searchCharacterModels(options: {
    ...
    return this.authenticatedRequest(
      `/search/character_models?${params.toString()}`
    );
  }
