...
const VrmModelPage = () => {
  ...
  const [testModel, setTestModel] = useState<VRoidCharacterModel>({
    ...
    view_count: 0,
    tags: [
      { name: 'test', locale: 'en', en_name: 'test', ja_name: 'テスト' }
    ],
    age_limit: {
      is_r18: false,
      is_r15: false,
  ...
  const fetchLikedModels = async () => {
    ...
    try {
      ...
      if (response.ok && data.data) {
        ...
        alert(`エラー: ${data.error?.message || 'いいねしたモデルの取得に失敗しました'}`);
      }
    } catch (error) {
        ...
        <CardContent className="space-y-2">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              {session && (
                <div className="mt-1 text-xs text-gray-600">
                  Provider: {session.provider || 'unknown'} | Token: {session.accessToken ? '✅' : '❌'}
                </div>
              )}
            </div>
            <div>
              <strong>VRoid連携:</strong>
              <Badge className={isVRoidLinked ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}>
            ...
            <div className="mt-3 p-3 bg-white rounded border">
              <h4 className="font-semibold text-sm mb-2">🎭 VRoidモデルフック状態:</h4>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                ...
                <div>
                  ...
                  <Badge className={vroidLoading ? "bg-yellow-100 text-yellow-800" : "bg-gray-100 text-gray-800"}>
                    {vroidLoading ? "🔄 読み込み中" : "⏹️ 停止中"}
                  </Badge>
                </div>
                <div>
                  <strong>エラー状態:</strong>
                  <Badge className={vroidError ? "bg-red-100 text-red-800" : "bg-green-100 text-green-800"}>
                    {vroidError ? "❌ エラーあり" : "✅ 正常"}
                  </Badge>
                </div>
              </div>
              {settings.selectedModel && (
                <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded text-xs">
                  <strong>選択中モデル:</strong> {settings.selectedModel.name} (ID: {settings.selectedModel.id})
                </div>
              )}
              {vroidError && (
            ...
            <div className="mt-3 p-2 bg-yellow-100 border border-yellow-300 rounded text-sm text-yellow-800">
              ...
              <a href="/login" className="ml-2 text-blue-600 underline">ログインページ</a>または
              <a href="/dashboard" className="ml-2 text-blue-600 underline">ダッシュボード</a>からVRoid連携を行ってください。
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="legacy" className="w-full">
        ...
        <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded">
          <div className="text-sm text-amber-800">
            ⚠️ <strong>API制限のお知らせ:</strong> 現在、VRoid Hub APIの個人開発者権限制限により、モデルID方式は制限されています。
        ...
        <TabsContent value="recommended" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                VRoidモデルIDを指定してURL直接読み込み + スマートキャッシュを使用
              </CardDescription>
            </CardHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    💡 ヒント: デバッグセクションの「いいねしたモデル一覧」から利用可能なモデルIDを確認できます
                  </p>
                </div>
                <div className="flex items-end">
                  <Button 
                    onClick={() => setTestModel({...testModel, id: customModelId})}
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  テスト用: <button 
                    ...
                    onClick={() => setCustomVrmUrl('https://3d.nicovideo.jp/alicia/alicia.vrm')}
                  >
                    サンプルVRM1
                  </button>
                  {' | '}
                  <button 
                    className="text-blue-500 underline" 
                    onClick={() => setCustomVrmUrl('https://raw.githubusercontent.com/vrm-c/vrm-samples/master/three-vrm-girl/three-vrm-girl.vrm')}
                  >
                    サンプルVRM2
                  </button>
                </p>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="border rounded-lg p-4">
                {testBlob ? (
                  <div>
                    <p className="text-sm text-muted-foreground mb-2">
                      ファイルサイズ: {(testBlob.size / 1024 / 1024).toFixed(2)} MB
                    </p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <ul className="space-y-1">
                <li>WebGL: {typeof WebGLRenderingContext !== 'undefined' ? '✅ 対応' : '❌ 非対応'}</li>
                <li>IndexedDB: {typeof indexedDB !== 'undefined' ? '✅ 対応' : '❌ 非対応'}</li>
                <li>User Agent: <code className="text-xs">{typeof navigator !== 'undefined' ? navigator.userAgent.slice(0, 50) + '...' : 'N/A'}</code></li>
              </ul>
            </div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t">
            ...
            <div className="flex gap-2 flex-wrap">
              <Button
                ...
                onClick={async () => {
                  try {
                    const response = await fetch('/api/vroid/download-license', {
                      method: 'POST',
              ...
              >
                {vroidLoading ? '🔄 取得中...' : '❤️ いいねモデル取得 (フック版)'}
              </Button>
              ...
              >
                {isLoadingLikedModels ? '🔄 取得中...' : '❤️ いいねモデル取得 (API版)'}
              </Button>
            </div>
            
            <div className="text-xs text-muted-foreground mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded">
              ⚠️ <strong>API制限について:</strong> 個人開発者アカウントでは一部のエンドポイントにアクセスできません。
          ...
          <div className="mt-4 pt-4 border-t">
            ...
            <div className="space-y-2 text-xs">
              <div className="p-3 bg-blue-50 border border-blue-200 rounded">
                <div className="font-semibold mb-2">📋 VRoid HubからモデルIDを取得する方法:</div>
                <ol className="list-decimal list-inside space-y-1 text-gray-700">
                  <li><a href="https://hub.vroid.com" target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">VRoid Hub</a> にアクセス</li>
                  <li>いいねしたモデルまたは使用したいモデルのページを開く</li>
                  <li>URLから数字のモデルIDをコピー（例: <code className="bg-gray-200 px-1 rounded">hub.vroid.com/characters/xxx/models/<strong>1234567890123456789</strong></code>）</li>
                  <li>上記の「モデルID」フィールドに貼り付けて「モデル更新」をクリック</li>
                </ol>
              </div>
              
              <div className="p-3 bg-green-50 border border-green-200 rounded">
                <div className="font-semibold mb-2">💡 テスト用おすすめモデルID:</div>
                <div className="space-y-1">
                  <button 
                    ...
                    onClick={() => {
                      ...
                      setTestModel({...testModel, id: testId, name: 'Test Model'});
                      alert(`テスト用モデルID ${testId} を設定しました！`);
                    }}
                  >
                    <code className="bg-blue-100 px-1 rounded">1070567262857269786</code> - 一般的なテストモデル
                  </button>
                </div>
              </div>
            </div>
          ...
          {vroidLikedModels.length > 0 && (
            <div className="mt-4 pt-4 border-t">
              <h4 className="font-semibold mb-2">❤️ いいねモデル一覧 (フック版) ({vroidLikedModels.length}件):</h4>
              <div className="max-h-60 overflow-y-auto space-y-2">
                {vroidLikedModels.map((model: VRoidCharacterModel, index: number) => (
                  <div key={model.id || index} className="p-3 bg-green-50 rounded border border-green-200 text-xs">
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <div className="font-semibold">モデル名: {model.name || '名前なし'}</div>
                        <div>モデルID: <code className="bg-blue-100 px-1 rounded text-blue-800">{model.id}</code></div>
                        <div>ダウンロード可: {model.is_downloadable ? '✅' : '❌'}</div>
                        <div>ハート数: {model.heart_count || 0}</div>
                      </div>
                      <div>
                        <div className="font-semibold">キャラクター情報:</div>
                        <div>キャラクター名: {model.character?.name || '不明'}</div>
                        <div>作成者: {model.character?.user?.name || '不明'}</div>
                        <div>プライベート: {model.character?.is_private ? '🔒' : '🌐'}</div>
                      </div>
                      <div className="flex flex-col gap-1">
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setTestModel({...testModel, id: model.id, name: model.name || 'Liked Model'});
                            updateSelectedModel(model); // VModelContextも更新
                            alert(`モデルID ${model.id} をテスト用に設定しました！`);
                          }}
                        >
                          🔧 テストに使用
                        </Button>
          ...
          {likedModels.length > 0 && (
            <div className="mt-4 pt-4 border-t">
              <h4 className="font-semibold mb-2">❤️ いいねモデル一覧 (API版) ({likedModels.length}件):</h4>
              <div className="max-h-60 overflow-y-auto space-y-2">
                {likedModels.map((model: any, index: number) => (
                  <div key={model.id || index} className="p-3 bg-gray-50 rounded border text-xs">
                    <div className="grid grid-cols-3 gap-2">
                      <div>
                        <div className="font-semibold">モデル名: {model.name || '名前なし'}</div>
                        <div>モデルID: <code className="bg-blue-100 px-1 rounded text-blue-800">{model.id}</code></div>
